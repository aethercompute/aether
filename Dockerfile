...
ENV LIBTORCH_USE_PYTORCH=1 \
    ...
    PYTHONUNBUFFERED=1 \
    RUST_LOG=info \
    STATE_PATH=/app/config/deepseek-v3-dense-160m-ultra-fineweb/state.toml \
    SERVER_PORT=39405 \
    WEB_PORT=8080
...
CMD ["/bin/sh", "-c", "export LD_LIBRARY_PATH=$(python3 -c \"import os, torch; print(os.path.join(os.path.dirname(torch.__file__), 'lib'))\"):${LD_LIBRARY_PATH}; exec psyche-centralized-server run --state \"${STATE_PATH}\" --server-port \"${SERVER_PORT}\" --web-port \"${WEB_PORT}\""]
